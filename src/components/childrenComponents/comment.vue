<template>
  <div style="height:37rem;">
    <el-card class="box-card" :body-style="bodyStyle" shadow="none">
      <p>
        <span style="font-size:1.5rem; padding-left: 10px; display:inline-block;">评论和回复</span>
      </p>
      <el-card
        class="box-card"
        shadow="none"
...
export default {
  ...
  created() {
    this.loading = true;
    this.$axios
      .get("/forward/comment/comment/search/findBySenderId", {
  ...
  methods: {
    deleteComment(id) {
      this.$axios
        .delete("/forward/comment/comment/" + id)
        .then(res => {
    ...
    checkBlog(comment) {
      ...
      this.$axios
        .get("/forward/blog/blog/"+comment.blogId)
        .then(res => {
          console.log(res);
